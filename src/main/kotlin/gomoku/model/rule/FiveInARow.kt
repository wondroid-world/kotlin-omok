package gomoku.model.rule

import gomoku.model.Board
import gomoku.model.Position
import gomoku.model.Row
import gomoku.model.Stone

class FiveInARow : Rule {
    override fun fiveInARow(
        position: Position,
        stone: Stone,
        board: Board,
    ): Boolean {
        val x = position.x
        val y = position.y

        // 가로
        var newX = x.value - 4
        var countX = 0
        repeat(8) {
            if (newX in 1..15) {
                if (board.stone(Position(Row(newX), y)) == stone) {
                    countX++
                    if (countX == 5) return true
                } else {
                    countX = 0
                }
            }
            newX++
        }

        // 세로
        var newY = y.value - 4
        var countY = 0
        repeat(8) {
            if (newY in 1..15) {
                if (board.stone(Position(x, Row(newY))) == stone) {
                    countY++
                    if (countY == 5) return true
                } else {
                    countY = 0
                }
            }
            newY++
        }

        // 대각선
        var newX1 = x.value - 4
        var newY1 = y.value - 4
        var countDiag1 = 0
        repeat(8) {
            if (newX1 in 1..15 && newY1 in 1..15) {
                if (board.stone(Position(Row(newX1), Row(newY1))) == stone) {
                    countDiag1++
                    if (countDiag1 == 5) return true
                } else {
                    countDiag1 = 0
                }
            }
            newX1++
            newY1++
        }

        // 대각선
        var newX2 = x.value - 4
        var newY2 = y.value + 4
        var countDiag2 = 0
        repeat(8) {
            if (newX2 in 1..15 && newY2 in 1..15) {
                if (board.stone(Position(Row(newX2), Row(newY2))) == stone) {
                    countDiag2++
                    if (countDiag2 == 5) return true
                } else {
                    countDiag2 = 0
                }
            }
            newX2++
            newY2--
        }

        return false
    }
}
